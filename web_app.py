...
def error_do(reason: str, value: str) -> None:
    ...
    put_markdown(t('TikHub_API 文档: [https://api.chatcm.top/docs](https://api.chatcm.top/docs)',
                   'TikHub_API Documentation: [https://api.chatcm.top/docs](https://api.chatcm.top/docs)'))
    ...
    put_markdown('[{}](https://web.chatcm.top/)'.format(
        t('嗨嗨', 'Click here to give feedback on GitHub')))
    put_html("<hr>")
...
def api_document_pop_window():
    with popup(t("API文档", "API Document")):
        ...
        put_link('[TikHub_API Docs]', 'https://api.chatcm.top/docs', new_window=True)
        put_html('<hr>')
        ...
        put_link('[API-V1 Docs]', 'https://web.chatcm.top/docs', new_window=True)
...
def log_popup_window():
    with popup(t('错误日志', 'Error Log')):
        ...
        put_markdown(t('该视频已经被删除或屏蔽(你看的都是些啥(⊙_⊙)?)',
                       'The video has been deleted or blocked (what are you watching (⊙_⊙)?)'))
        put_markdown(t('[嗨嗨](https://web.chatcm.top/)',
                       '[Click here to feedback on GitHub](https://web.chatcm.top/)'))
        put_html('<hr>')
        # 判断日志文件是否存在
...
def about_popup_window():
    with popup(t('更多信息', 'More Information')):
        ...
        put_markdown('{}：[issues](https://web.chatcm.top/)'.format(
...
async def main():
    # 设置favicon
    favicon_url = "https://xmx-1308863530.cos.ap-chengdu.myqcloud.com/IMG_20230405_191454.jpg"
    ...
    put_html(f"""
    <div align="center">
    <a href="https://web.chatcm.top/" alt="logo" ><img src="{favicon_url}" width="100"/></a>
